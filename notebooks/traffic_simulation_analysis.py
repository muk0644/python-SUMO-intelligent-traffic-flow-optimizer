...
class SUMOTrafficSimulator:
    ...
    def visualize_network(self):
        """
        Create a visualization of the network/routes
        """
        ...
        try:
            ...
            edge_names = [e for e in edges if not e.startswith(':')]
            
            # Create simple network diagram
            fig, ax = plt.subplots(figsize=(12, 8))
            
            colors = ['#3498db' for _ in edge_names]
            ax.barh(edge_names, [1]*len(edge_names), color=colors, alpha=0.7, height=0.6)
            
            ax.set_xlabel('Network Edges', fontsize=12, fontweight='bold')
            ax.set_title('Traffic Network Routes', fontsize=14, fontweight='bold', pad=20)
            ax.set_xlim(0, 1.5)
            ...
            routes_path = os.path.join('results', 'routes.png')
            plt.savefig(routes_path, dpi=300, bbox_inches='tight')
            print(f"🗺️  Network routes saved to: {routes_path}")
            plt.close()
...
def main():
    ...
    parser.add_argument('--config', '-c', default="simulation/sumo_config.sumocfg",
                        help='Path to SUMO configuration file')
    parser.add_argument('--sumo-home', dest='sumo_home', default="C:/Program Files (x86)/Eclipse/Sumo",
                        help='SUMO_HOME path')
